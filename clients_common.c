...
int read_request(struct client *client) {
	...
	// Here's a VERY BAD version of this function
	...
	if(header_complete(client->buffer, client->nread)) {
		...
		//No error treatment!
		get_filename(client->buffer, client->nread, filename, 1024, protocol, 16);
...
int write_reply(struct client *client) {
	...
	// What follows is a VERY BAD version of this function that you should improve!

	// That's a "courageous" call with no error treatment. Ugh.
	// Also, you may write only half of the header. What happens then?
	write(client->socket, client->buffer, client->ntowrite);

	// If the file isn't NULL, we just sent a "200 OK" response, and now we need to send the file
	if(client->file != NULL) {
		// If the file size is not multiple of BUFFER_SIZE, we are sending junk!
		// No error treatment for write: should use flush_buffer
		while(fread(client->buffer, sizeof(char), BUFFER_SIZE, client->file) > 0) {
			write(client->socket, client->buffer, BUFFER_SIZE);
		}
	}
